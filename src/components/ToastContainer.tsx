...
function ToastRow({item}: {item: ToastItem}) {
  ...
  return (
    <Animated.View style={[styles.toast, {backgroundColor: color.bg, opacity, transform: [{translateY}]}]}>
      ...
      <View style={[styles.textWrap, {fontFamily: MONO}]}>
        <Text style={[styles.title, {fontFamily: MONO}]}>{item.title}</Text>
        {item.message ? <Text style={[styles.message, {fontFamily: MONO}]}>{item.message}</Text> : null}
      </View>
